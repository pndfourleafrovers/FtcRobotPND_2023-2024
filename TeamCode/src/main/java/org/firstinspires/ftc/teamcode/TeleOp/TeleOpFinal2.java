...
@TeleOp(name="TeleOpFinal2", group="TeleOp")
//@Disabled
...
*/
    ...
    public void turnToHeading(int targetHeading) {
        double turnKp = 0.015; // Proportional gain
        double minTurnPower = 0.1; // Minimum power

        while (opModeIsActive()) {
            double currentHeading = getHeading(); // Get current heading
            double error = targetHeading - currentHeading;

            // Normalize the error to handle the 180/-180 transition
            while (error > 180)  error -= 360;
            while (error <= -180) error += 360;

            if (Math.abs(error) <= 1) break; // Exit condition

            double turnPower = error * turnKp;
            turnPower = Math.copySign(Math.max(Math.abs(turnPower), minTurnPower), turnPower);
            ...
            telemetry.addData(">", "Robot Heading = %4.0f", currentHeading);
            telemetry.update();
        }
